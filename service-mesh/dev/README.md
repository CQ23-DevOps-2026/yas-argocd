...
* `04-authorization-policy.yaml`: Adds public entry allow rules, public OpenAPI docs access, and service-to-service allow rules for the core product/search/cart/order/checkout flow.
...
The basic AuthorizationPolicy keeps ingress-facing services (`storefront-bff`, `backoffice-bff`, `swagger-ui`, and `media`) public through selector-scoped allow rules. OpenAPI docs are allowed through a namespace-level `public-api-docs` policy so Swagger UI can load `/v3/api-docs`. Internal backend services are protected with real application service accounts.
